...
try {
  ...
  $repoSha = ($rows | Where-Object { $_[0] -eq 'repo' -and $_[1] -eq 'sha256' })[0][2]
  ...
  $onDisk = (Get-FileHash -Algorithm SHA256 '.repo\manifests\bootstrap-pins.txt').Hash
  if ((Get-FileHash -Algorithm SHA256 $pins).Hash -ne $onDisk) {
  ...
  & (Join-Path $pixiBin 'pixi.exe') install --manifest-path '.repo\manifests\pixi.toml' --all
